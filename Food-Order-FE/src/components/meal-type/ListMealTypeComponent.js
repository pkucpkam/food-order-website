...
const ListMealTypeComponent = () => {
    ...
    const handleSubmitEdit = () => {
        ...
        fd.delete('image'); // Xóa dữ liệu cũ nếu có
        fd.delete('mealType');
        ...
        if (selectedFile) {
            ...
            // Nếu không có ảnh mới, gửi giá trị rỗng
            fd.append('image', '');
        }
    
        // Gọi hàm cập nhật mealType từ MealTypeService
        MealTypeService.updateMealType(fd).then((response) => {
            const responseFromServer = response.data;
            if (responseFromServer === "success") {
                alert("Successfully updated meal type!");
                handleCloseEdit();
                getAllMealTypes(); // Làm mới danh sách sau khi cập nhật thành công
